(() => {
  ...
  function preload() {
    bgImg = loadImage(
      ...
      () =>
        console.error("Failed to load background image. Using fallback colors.")
    );
  ...
  function displayStartScreen() {
    ...
    fill(230, 230, 230);
    textSize(18);
    text("Jump over obstacles and collect lives!", width / 2, height / 2 + 100);
  ...
  function manageMusic() {
    if (gameState === "startScreen") {
      ...
      if (
        gameplaySong &&
        gameplaySong.isLoaded() &&
        !gameplaySong.isPlaying()
      ) {
        gameplaySong.loop();
      }
    }
  ...
  // Automatically play the title music once the page loads
